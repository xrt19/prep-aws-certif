# AWS Solutions Architect Associate (SAA-C03)

Study workspace untuk persiapan sertifikasi AWS SAA-C03.

## Exam Overview

| Item | Detail |
|------|--------|
| Exam Code | SAA-C03 |
| Durasi | 130 menit |
| Jumlah soal | 65 soal (multiple choice & multiple response) |
| Passing score | 720 / 1000 |
| Biaya | $150 USD |
| Format | Pearson VUE / Online proctored |

## Domain & Bobot

| # | Domain | Bobot |
|---|--------|-------|
| 1 | Design Secure Architectures | 30% |
| 2 | Design Resilient Architectures | 26% |
| 3 | Design High-Performing Architectures | 24% |
| 4 | Design Cost-Optimized Architectures | 20% |

## Struktur Folder

```
aws certif/
├── 01-secure-architectures/     # Domain 1 — bobot terbesar
├── 02-resilient-architectures/  # Domain 2
├── 03-high-performing/          # Domain 3
├── 04-cost-optimized/           # Domain 4
├── cheatsheets/                 # Quick reference per service
├── practice-questions/          # Latihan soal & pembahasan
└── progress.md                  # Tracking progress belajar
```

## Resources

- [AWS SAA-C03 Exam Guide (official)](https://d1.awsstatic.com/training-certification/docs-sa-assoc/AWS-Certified-Solutions-Architect-Associate_Exam-Guide.pdf)
- [AWS Free Tier](https://aws.amazon.com/free/) — untuk hands-on practice
- [Tutorials Dojo Practice Exams](https://tutorialsdojo.com/aws-certified-solutions-architect-associate/) — highly recommended
- [Stephane Maarek – Udemy Course](https://www.udemy.com/course/aws-certified-solutions-architect-associate-saa-c03/) — paling populer
